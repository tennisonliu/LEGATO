'''
Evaluating the impact of local correlation on learning.
'''
import sys
sys.path.append("../")
sys.path.append("../..")
...
setting = 'local'
exp_name = f'sim_corr_{setting}'
SEEDS = 5
batch_size = 64
DEVICE = 'cuda'

# simulation setting
corr_ls = [0.25, 0.5, 0.75]
num_views = 4
num_samples = 1000
input_dim = 100

print(f'investigating effect of correlation on multi-view learning, with p in {corr_ls}...')
...
for corr in corr_ls:
    ...
    print(f'tuning hyperparameters for setting with local corr: {corr}')
    ...
    for SEED in range(SEEDS):
        ...
        print(f'processing setting with local corr: {corr}, run {SEED+1}...')
        ...
        print(f'[local corr {corr}, run {SEED+1}] test loss: {test_loss:.4f}')
        print('\n')
...
save_results(pretrain_loss_ls, finetune_loss_ls, test_loss_ls, corr_ls, fname)