'''
This script runs the uci experiment for 10 times and saves the results in ../results/uci_acc_LEGATO.json
'''
...
sys.path.append("../..")
import numpy as np
from load_uci import get_dataloaders
...
print(f'running experiments on uci datasets for {SEEDS} runs')
...
for SEED in range(SEEDS):
    ...
    optimiser = optim.Adam(
        model.parameters(), lr=len(input_dims)*lr, weight_decay=weight_decay)
    ...
    downstream_model = LinearProbe(
        encoder=model, n_in_feats=model.latent_dims, n_targets=10, freeze_encoder=False).to(DEVICE)
