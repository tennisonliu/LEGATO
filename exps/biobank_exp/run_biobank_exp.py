'''
This script runs the UK Biobank experiment for 10 times and saves the results in ../results/biobank_acc_LEGATO.json
'''
...
sys.path.append("../..")
import numpy as np
from load_biobank import get_dataloaders
...
exp_name = f'biobank_acc'
SEEDS = 10
batch_size = 64
input_dims = [15, 67, 17, 33, 103, 1086, 113, 80, 121]
...
print(f'running experiments on biobank datasets for {SEEDS} runs')
...
for SEED in range(SEEDS):
    ...
    downstream_model = LinearProbe(encoder=model, n_in_feats=model.latent_dims, n_targets=1, freeze_encoder=False).to(DEVICE)

    criterion = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([7.904761904761905]).to(DEVICE))
